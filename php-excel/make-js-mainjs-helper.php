...
// TODO
$tool->extend73X(110, '      {sheet: 1, row: 110, col: 3, json: styleSubTotal({data: "=C108+C109"})},  ');
...
class MarkToolMainJs {
    ...
    public function extend67() {
        echo "<br><br>// --- extend67() ---<br>";
        $row = ' {sheet: 1, row: 67, col: 3, json: {fm: "money|¥|2|none", dsd: "ed", cal: true, data: "=VLOOKUP(C65,LOOKUP04!$A$2:$B$17,2,0)"}},';
        $seq = "0ABCDEFGH";
        //  $arr[] = "C65";
        for ($k = 4; $k <= 8; $k++) {
            $strD = str_replace("col: 3", "col: $k", $row);
            $colName = substr($seq, $k, 1);
            $strD = str_replace("C65", $colName . "65", $strD);
            echo $strD . "<br>";
        }
    }

    public function extend69() {
        echo "<br><br>// --- extend69() ---<br>";
        $row = '    {sheet: 1, row: 69, col: 3, json: styleSubTotal({data: setNaToZero("(C66/3600)*C67 * (1 + (1 - C68 / 100))")})},';
        echo $row . "<br>";
        $seq = "0ABCDEFGH";
        $arr[] = 66;
        $arr[] = 67;
        ...
        for ($k = 4; $k <= 8; $k++) {
            ...
            $colName = substr($seq, $k, 1);
            foreach ($arr as $item) {
                $strD = str_replace("C$item", $colName . $item, $strD);
            }
//            $strD = str_replace("C67", $colName."67", $strD);
//            $strD = str_replace("C68", $colName."68", $strD);

            echo $strD . "<br>";
        }
    }

    public function extend72() {
        echo "<br><br>// --- extend72() ---<br>";
        $row = '        {sheet: 1, row: 72, col: 3, json: {fm: "money|¥|2|none", dsd: "ed", cal: true, data: "=VLOOKUP(C70,LOOKUP04!$A$2:$B$17,2,0)"}},      ';
        echo $row . "<br>";
        $seq = "0ABCDEFGH";
    ...
    public function extend73X($rowNum, $row) {
        echo "<br><br>// ---  extend73X($rowNum,$row) ---<br>";
        //  $row = '        {sheet: 1, row: 72, col: 3, json: {fm: "money|¥|2|none", dsd: "ed", cal: true, data: "=VLOOKUP(C70,LOOKUP04!$A$2:$B$17,2,0)"}},      ';
        echo $row . "<br>";
        $seq = "0ABCDEFGH";
